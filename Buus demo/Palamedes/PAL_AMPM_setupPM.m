...
NumOpts = length(varargin); % =14; 

%Alt i nedenstående if loop er total ligegyldigt. 
if mod(NumOpts,2) == 0 %mod: modulo 
    %PRIOR DIST
    ...
    %4D matrice with prior valeues (size of PM.priorAlphas  81    41)
    ...
    %LUT (Look-up Table) is 5D likelihood matrice (PRIOR ?) - size: 81,41,1,1,21
    ...
    %PDF: Identical probabily on all parameter combination. 
    %2D matrice with ones. 
...
if NumOpts > 1
    ...
    %load the different parameter in PM.xxx
    for n = 1:2:NumOpts-mod(NumOpts,2)  % 1     3     5     7     9    11    13
        n = n+mod(NumOpts,2);
        valid = 0;
        if strncmpi(varargin{n}, opts(1),6)            
            PM.priorAlphaRange = varargin{n+1}; % <---- prior alpha range laod               
            valid = 1;
            supplied(1) = true;
        end
        ...
        if strncmpi(varargin{n}, opts(12),6) %stemmer de 6 første tegn ens mellem de to strings
    ...
    if supplied(1) || supplied(2) || supplied(3) || supplied(4)
        %Resave as a matrice instead of vectors. 
        [PM.priorAlphas, PM.priorBetas, PM.priorGammas, PM.priorLambdas] = ndgrid(PM.priorAlphaRange,PM.priorBetaRange,PM.priorGammaRange,PM.priorLambdaRange);
        if ~supplied(6)
            PM.prior = ones(size(PM.priorAlphas)); %4d   
            PM.prior = PM.prior./sum(sum(sum(sum(PM.prior)))); % sum ... = 40401 (nummer of elements)

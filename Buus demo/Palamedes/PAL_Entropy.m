...
if ~isempty(varargin)
    ...
    if length(varargin) > 1
        % sort; arrange number in an accending order - just ignore
        % fliplr; Flip array in left/right direction - just ignore
        
        marginalize = fliplr(sort(varargin{2}));
        for dim = 1:length(marginalize);  % 1 --> 4
            %disp('pdf before marginalize')
            %pdf
            pdf = sum(pdf,marginalize(dim));
            %disp('pfd after marginalize') 
            %pfd
        end
    end
end

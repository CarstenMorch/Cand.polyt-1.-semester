...
%PFLookUpTable:  Look-up Table (LUT) is 5D likelihood matrice 
%PDF: Identical probabily on all parameter combination (2D matrice with ones)

%Gentag pdf, for 1 x 1 x 1 x 1 x X dimensioner
pdf5D = repmat(pdf, [1 1 1 1 size(PFLookUpTable,5)]);
pSuccessGivenx = sum(sum(sum(sum(pdf5D.*PFLookUpTable,1),2),3),4);

PosteriorTplus1givenSuccess = pdf5D.*PFLookUpTable; % p(succes/lambda, x)
PosteriorTplus1givenFailure = pdf5D-PosteriorTplus1givenSuccess;

if exist('bsxfun.m','file') %Intern matlab func
